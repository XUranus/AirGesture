...
class GrabDropDesktop:
    def __init__(self):
        ...
        self.camera_preview = CameraPreview() if config.SHOW_CAMERA_PREVIEW else None
    ...
    def start(self):
        ...
        if self.camera_preview:
            ...
            self.logger.info("📷 Camera preview window opened")
        ...
        # Connect camera preview to gesture detector
        if self.camera_preview:
            self.camera_preview.on_frame = self.gesture_detector.process_frame
            self.gesture_detector.start(use_shared_camera=True)
        else:
            self.gesture_detector.start(use_shared_camera=False)
        ...
        try:
            ...
            if system == "Linux":
                ...
                try:
                    ...
                    # 手上滑 → 页面向下滚（看到下面内容）→ 负值
                    # 手下滑 → 页面向上滚（看到上面内容）→ 正值
                    amount = -120 * 3 if direction == "up" else 120 * 3
