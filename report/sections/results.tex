...
and F1-score of the deployed model (pruned and quantized) on the test set.

\begin{table}[ht]
\centering
\caption{Classification report on the test set (deployed model).}
...
\texttt{swipe\_up}   & 0.667 & 0.800 & 0.727 & 5 \\
\texttt{swipe\_down} & 1.000 & 1.000 & 1.000 & 5 \\
\texttt{noise}       & 0.857 & 0.857 & 0.857 & 7 \\
\midrule
\textbf{Overall accuracy} & \multicolumn{3}{c}{89.66\%} & 29 \\
\textbf{Macro F1} & \multicolumn{3}{c}{0.899} & \\
\textbf{Weighted F1} & \multicolumn{3}{c}{0.900} & \\
\bottomrule
\end{tabular}
\end{table}

The model achieves perfect performance on \texttt{release} and
\texttt{swipe\_down}, while \texttt{swipe\_up} shows lower precision
(0.667) due to confusion with other classes. The \texttt{grab} class
achieves high precision (1.000) but slightly lower recall (0.833),
with one sample misclassified. These errors are understandable given
the spatial and temporal similarity between these gesture types.
The overall accuracy of 89.66\% reflects the challenge of
distinguishing similar hand motions from limited test data.
...
Grabdrop has been tested across two device configurations:
Android phones paired with Windows desktops, and Android phones paired
with Linux desktops. In both cases, device discovery, gesture
