...
exporter. The export specifies:
\begin{itemize}[leftmargin=*]
  \item \textbf{Dynamic batch axis}: Allows variable batch sizes at
    inference time.
  \item \textbf{Opset version 18}: Ensures compatibility with modern
    ONNX Runtime versions.
  \item \textbf{Named I/O}: Input tensor named \texttt{``input''}
    (shape $[B, 144, 30]$), output tensor named \texttt{``logits''}
    (shape $[B, 5]$).
\end{itemize}
...
signed 8-bit integers. The process consists of:

\begin{enumerate}[leftmargin=*]
  \item \textbf{Calibration}: Real training data is fed through the
    FP32 ONNX model to observe the range of activation values at each
    layer.
  \item \textbf{Scale computation}: Per-tensor scale and zero-point
    values are computed to map FP32 ranges to INT8:
    $x_{\text{int8}} = \text{round}(x_{\text{fp32}} / s) + z$.
  \item \textbf{QDQ insertion}: Quantize-Dequantize (QDQ) nodes are
    inserted into the computation graph, enabling the runtime to fuse
    quantized operations for optimal performance.
\end{enumerate}
