...
stochastic augmentation at each access: coordinate jitter with standard deviation $\sigma = 0.002$ is applied with 50\% probability, and time warping is applied with 30\% probability.
This means the model sees slightly different variants of each sample
every epoch, further improving generalization.

\subsection{Training Dynamics}

Training terminated after 49~epochs via early stopping, with the best
test accuracy of 89.66\% achieved at epoch~9.
...
The rapid convergence (best model at epoch~9) suggests the model
quickly learns the discriminative features, with subsequent epochs
showing signs of overfitting as training accuracy reaches 100\%
while test accuracy fluctuates between 82.76\% and 89.66\%.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.95\linewidth]{training_curves}
  \caption{Training curves: loss (left) and accuracy (right) over
    49~epochs. Early stopping was triggered at epoch~49 with the best
